	...
	$.fn.tablefix = function(options) {
		return this.each(function(index){
			...
			// スクロール部オフセットの取得
                        ...
                        var fixHeaders = baseTable.find("thead").children().length;
                        var fixFooters = baseTable.find("tfoot").children().length;

			var fixRows = fixHeaders + fixFooters;
			...
			var bodyTable = baseTable.clone().wrap('<div></div>');
                        var crossfTable = baseTable.clone().wrap('<div></div>');
			...
			var bodyDiv = bodyTable.parent().css({position: "absolute", overflow: "auto"});
                        ...
                        var crossfDiv = crossfTable.parent().css({position: "absolute", overflow: "hidden"});
                        var footDiv = footTable.parent().css({position: "absolute", overflow: "hidden"});

			div.append(rowDiv).append(colDiv).append(bodyDiv).append(crossfDiv).append(footDiv);
...
*/
			...
			bodyTable.css({
				...
				marginBottom: (withHeight ? 20 : 0) + 'px'
                        ...
                        footDiv
                              .width(bodyWidth + (withWidth ? 20 : 0) + (withHeight ? 20 : 0))
                              .height(offsetY)
                              .top(odyHeight + (withWidth ? 20 : 0) + (withHeight ? 20 : 0))
                              .css({left: offsetX + 'px', top: offsetY + 'px'});                        
			...
			bodyDiv.scroll(function() {
				...
				colDiv.scrollTop(bodyDiv.scrollTop());
                                footDiv.scrollLeft(bodyDiv.scrollLeft());
			});
